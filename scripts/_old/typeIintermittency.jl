...
function plot_time_series_type_I_intermittency_logistic(r=3.8284; T = 1000, Ttr = 0)
    lo = Systems.logistic(0.4; r); t = Ttr:Ttr+T
    traj = trajectory(lo, T; Ttr);

    Ts, corr_lam_periods_bool= logistic_laminarperiods(traj, 3; atol=0.0, rtol=0.02)
    ...
    ylims!(0, 1)
    xlims!(0, 500)
    filename = "$(plotsdir())/mechanisms/type_I_intermittency/logistic/logistic-timeseries-r_$(r).png"
    mkpath(dirname(filename))
    save(filename, fig, px_per_unit=3)
    return fig, ax
end

fig, ax = plot_time_series_type_I_intermittency_logistic()
