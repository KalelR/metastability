...
T = 10000
Δt = 0.01
#decreasing K (coupling) increases instability, leads to more fluctuations
# K = 0.045
K = 0.04
# K = 0.035
# K = 0.02
...
c1 = :purple
c2 = :green
fig = Figure(resolution=(3.0*columnsize_pt, 2.2*columnsize_pt)) #subplotgrid(2, 1; resolution=(2.0*columnsize_pt, 1.0*columnsize_pt), rowtitles, xlabels="t", ylabels="x", sharex=true, sharey=true)
axs = []
panel_idxs = [1:2, 3:4, 5:6]
distances = colwise(Euclidean(), tr[:, 1:3], tr[:, 4:6])
# distances = mapslices(v->norm(v), Matrix(tr[:, 4:6]) .- Matrix(tr[:, 1:3]), dims=2)[:,1]
colors = [distance > 1 ? c1 : c2 for distance in distances]
for i=1:3
	ax = Axis(fig[1, panel_idxs[i]], ylabel=["δx", "δy", "δz"][i], xlabel="t"); push!(axs, ax)
	δvar = tr[:, i+3] .- tr[:, i]
	lines!(ax, t, δvar, color=colors)
end

ax = Axis(fig[2, :], ylabel="distance", xlabel="t")
lines!(ax, t, distances, color = colors)

ax = Axis3(fig[3, 1:3], viewmode=:stretch)
lines!(ax, tr[:, 1], tr[:,2], tr[:,3], color=colors)

ax = Axis3(fig[3, 4:6], xlabel="δx", ylabel="δy", zlabel="δz", viewmode=:stretch)
lines!(ax, tr[:, 4] .- tr[:, 1], tr[:, 5] .- tr[:,2], tr[:,6] .- tr[:,3], color=colors)

rowsize!(fig.layout, 3, Relative(0.6))
...
save("$(plotsdir())/mechanisms/onoffintermittency/onoff-intermittency-coupledrosslers-K_$(K).png", fig, px_per_unit=4)
fig
end